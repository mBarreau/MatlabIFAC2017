%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%  Synthesys of an observer for a system with an average on the state   %%
%                                                                         %
% By M. Barreau                                                           %
%                                                                         %
...
for alpha = alphaStart:alphaStep:alphaMax
    ...
    i = 0;
    while (1)
        [pres,K] = observer(A, C, h, e1, e2, e3, e4, alpha);
        if(sum(pres > 0) == length(pres)) % There exists a controler for this (h,alpha)
            ...
            if hmin >= 0 % it was feasible for a smaller h
                ...
                if i == nbDecimal % If we get the maximum number of decimal, we stop
                    display('hmax found');
                    hmax = h;
                    [pres,K,gamma] = observer(A, C, hmax, e1, e2, e3, e4, alpha);
                    break
                end
                i = i + 1;
